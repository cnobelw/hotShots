...
void MainWindow::saveSettings()
{
    ...
    settings.setValue( "proxyPassword",  MiscFunctions::base64Encode(m_proxiParams.proxyPassword) ); // in order to store unreadable password
...
void MainWindow::loadSettings()
{
    ...
    m_systrayUserActionList = settings.value("systrayUserActionList",QStringList() << "actionAddAnnotations").toStringList();
    ...
    m_proxiParams.proxyPassword = MiscFunctions::base64Decode( settings.value("proxyPassword","").toString() ); // decode encode password
...
void MainWindow::processNewSnapshot(const QPixmap &pix)
{
    ...
    if (m_scaleImage)
    {
        ...
        {
            m_currPixmap = pix.scaled(pix.size() * (m_scaleValue / 100.0),Qt::KeepAspectRatio,Qt::SmoothTransformation);
        }
        else
        {
...
{
    ui->labelPreview->setPixmap( pix.scaled(ui->labelPreview->size(),
                                            Qt::KeepAspectRatio,
...
void MainWindow::uploadToWeb(const QPixmap &pix, const QString &preferredName)
{
    ...
    if ( !up->isValid() )
    {
        QMessageBox::information( this, tr("Uploader %1").arg( up->getName() ),
                                  tr("The default uploader (%1) does not appear correctly configured, no value given for required parameters!!").arg( up->getName() ) );
        return;
    }
    ...
    if (m_showDialogBeforeDownload)
    {
        ...
        msgBox.setInformativeText( tr("Upload image %1 to %2 service?").arg( QFileInfo(saveName).baseName() ).arg( up->getName() ) );
...
bool MainWindow::saveSnapshot(const QPixmap &pix, const QString &file)
{
    ...
    if ( !QFileInfo(file).baseName().startsWith(NameManager::lastSnapshotName) )
    {
        if (NameManager::addDateTime)
            NameManager::lastSnapshotName = QFileInfo(file).baseName().replace(QString("-%1").arg(NameManager::lastDateTime),"");
        else
            NameManager::lastSnapshotName = QFileInfo(file).baseName().replace(QString("-%1").arg(NameManager::currentSnapshotNum),"");
    }
...
void MainWindow::initEditor()
{
    if (!m_snapshotEditor)
    {
        ...
        connect( m_snapshotEditor,SIGNAL( requestUpload(const QPixmap &, const QString &) ),this,SLOT( uploadToWeb(const QPixmap &, const QString &) ) );
        connect( m_snapshotEditor,SIGNAL( requestExport(const QPixmap &, const QString &) ),this,SLOT( exportToDisk(const QPixmap &, const QString &) ) );
